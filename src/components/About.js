...
export default function TypingHeading() {
 const { ref, inView } = useInView({ threshold: 0.5 });
  ...
  useEffect(() => {
    ...
    <section className='flex flex-col md:flex-row border-2 border-lightlaven  shadow-2xl rounded-2xl p-12 md:p-20 '  >
        ...
        <div className='flex flex-col  md:pl-8 '>
            <h1 ref={ref} className='text-[#7596F7] text-2xl md:text-5xl  font-cal  pb-2 tracking-wide'>
                  {inView && (
        <Typewriter
        ...
        />
            ...
            <p className='pt-12 text-1xl font-mono '> {config.line1}</p>
            <p className='pt-6 text-1xl font-mono'>{config.line2} </p>
            <p className='pt-6 text-1xl font-mono '>{config.line3}</p>
            </div>
        </div>
    </section>
    </div>
